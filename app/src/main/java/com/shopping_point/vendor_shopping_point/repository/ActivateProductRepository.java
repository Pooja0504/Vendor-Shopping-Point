...
public class ActivateProductRepository {

    private static final String TAG = ActivateProductRepository.class.getSimpleName();
    private Application application;

    public ActivateProductRepository(Application application) {
        this.application = application;
    }

    public LiveData<ResponseBody> activateProduct(int product_id) {
        final MutableLiveData<ResponseBody> mutableLiveData = new MutableLiveData<>();
        RetrofitClient.getInstance().getApi().activateProduct(product_id).enqueue(new Callback<ResponseBody>() {
            @Override
